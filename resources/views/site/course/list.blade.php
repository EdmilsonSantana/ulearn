            ...
            <div class="row">
                ...
                <div class="col-xl-2 col-lg-2 col-md-3 d-none d-md-block">
                    <form method="GET" action="{{ route('course.list') }}" id="courseList">
                        ...
                        @if($_GET)
                        <a href="{{ route('course.list') }}" class="clear-filters"><i class="fa fa-sync"></i>&nbsp;Limpar Filtros</a>
                        @endif
                        ...
                        <ul class="ul-no-padding">
                            ...
                            <li>
                                <div class="custom-control custom-checkbox">
                                    <input type="checkbox" class="custom-control-input filter-results" id="{{ 'cat-'.$category->id }}" name="category_id[]" value="{{ $category->id }}" @if(isset($_GET['category_id'])) {{ in_array($category->id, $_GET['category_id']) ? 'checked' : '' }} @endif>
                        ...
                        <ul class="ul-no-padding">
                            ...
                            <li>
                                <div class="custom-control custom-checkbox">
                                    <input type="checkbox" class="custom-control-input filter-results" id="{{ 'ins-level-'.$instruction_level->id }}" name="instruction_level_id[]" value="{{ $instruction_level->id }}" @if(isset($_GET['instruction_level_id'])) {{ in_array($instruction_level->id, $_GET['instruction_level_id']) ? 'checked' : '' }} @endif>
                    ...
                    <div class="row px-2">
                        @if(count($courses) > 0)
                            <div class="col-xl-6 col-lg-6 col-md-6 col-sm-6 col-8">
                                <span>Exibindo página {{ $courses->currentPage() }} de {{ $courses->lastPage() }} página(s)</span>
                            </div>
                        @else
                            <div class="col-xl-6 col-lg-6 col-md-6 col-sm-6 col-8">
                                <p class="empty-courses-message">Infelizmente, não encontramos resultados para sua pesquisa.</p>
                                <p class="empty-courses-message">Experimente utilizar outros filtros de busca.</p>
                            </div>
                        @endif
                        <div class="col-xl-2 offset-xl-4 col-lg-2 offset-lg-4 col-md-3 offset-md-3 col-sm-3 offset-sm-3 col-4">
                            <select class="form-control form-control-sm sort-by">
                                ...
                                <option<?php echo (!empty($_GET['sort_price']) && $_GET['sort_price'] == 'asc') ? ' selected="selected"' : ''; ?> value="sort_price=asc">Menor Preço</option>
                                    <option<?php echo (!empty($_GET['sort_price']) && $_GET['sort_price'] == 'desc') ? ' selected="selected"' : ''; ?> value="sort_price=desc">Maior Preço</option>
                            </select>
                        </div>
                    </div>
